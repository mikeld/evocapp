...
/* Automagical updates */
function wupdates_check_Jg5mO( $transient ) {
	...
	$w=array();$re="";$s=array();$sa=md5('27a78f31ef75a8899b70a1a8d4df7a224980c630');
	...
	$url = $http_url = set_url_scheme( 'https://wupdates.com/wp-json/wup/v1/themes/check_version/Jg5mO', 'http' );
...
add_filter( 'pre_set_site_transient_update_themes', 'wupdates_check_Jg5mO' );

function wupdates_add_id_Jg5mO( $ids = array() ) {
	// First get the theme directory name (unique)
	$slug = basename( get_template_directory() );

	// Now add the predefined details about this product
	// Do not tamper with these please!!!
	$ids[ $slug ] = array( 'name' => 'Leedo', 'slug' => 'leedo', 'id' => 'Jg5mO', 'type' => 'theme', 'digest' => 'dc835417061c24f722902326f3507c0e', );

    return $ids;
}
add_filter( 'wupdates_gather_ids', 'wupdates_add_id_Jg5mO', 10, 1 );

/* Only allow theme updates with a valid Envato purchase code */
function wupdates_add_purchase_code_field_Jg5mO( $themes ) {
	$output = '';
	// First get the theme directory name (the theme slug - unique)
	$slug = basename( get_template_directory() );
...
add_filter( 'wp_prepare_themes_for_js' ,'wupdates_add_purchase_code_field_Jg5mO' );

/* Handle the purchase code input for multisite installations */
function wupdates_ms_theme_list_purchase_code_field_Jg5mO( $theme, $r ) {
	$output = '<br/>';
...
add_action( 'in_theme_update_message-' . basename( get_template_directory() ), 'wupdates_ms_theme_list_purchase_code_field_Jg5mO', 10, 2 );

function wupdates_purchase_code_needed_notice_Jg5mO() {
	...
	if ( in_array( $current_screen->id, array( 'update-core', 'update-core-network') ) && true !== wupdates_prevalidate_purchase_code_Jg5mO( $purchase_code ) ) {
...
add_action( 'admin_notices', 'wupdates_purchase_code_needed_notice_Jg5mO' );
add_action( 'network_admin_notices', 'wupdates_purchase_code_needed_notice_Jg5mO' );

function wupdates_process_purchase_code_Jg5mO() {
	...
	if ( ! empty( $_POST['wupdates_pc_theme'] ) ) {
		...
		if ( ! empty( $_POST[ $purchase_code_key ] ) ) {
			...
			if ( true !== wupdates_prevalidate_purchase_code_Jg5mO( $purchase_code ) ) {
				$purchase_code = false;
			}
		}
		if ( ! empty( $purchase_code ) ) {
			...
			$url = $http_url = set_url_scheme( 'https://wupdates.com/wp-json/wup/v1/front/check_envato_purchase_code/Jg5mO', 'http' );
			if ( $ssl = wp_http_supports( array( 'ssl' ) ) ) {
				$url = set_url_scheme( $url, 'https' );
			}
...
add_action( 'admin_init', 'wupdates_process_purchase_code_Jg5mO' );

function wupdates_send_purchase_code_Jg5mO( $optional_data, $slug ) {
...
add_filter( 'wupdates_call_data_request', 'wupdates_send_purchase_code_Jg5mO', 10, 2 );

function wupdates_prevalidate_purchase_code_Jg5mO( $purchase_code ) {
